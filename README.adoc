...
Pinpointed when and where a request is received, as well as when responses are sent back become difficult.
...
Before you proceed, confirm your Zipkin server is up and running. 
...
Before you proceed, make sure your Zipkin server is up and running.
...
To enable tracing, add the `@Traced` annotation to the `InventoryManager` class.
...
Then, a [hotspot=childScope file=0]`childScope` is created, activated, 
and used the [hotspot=Try file=0]`try` block.
...
Setting [hotspot=Traced-false file=1]`@Traced(false)` disables tracing of 
the [hotspot=listContents file=1]`listContents()` JAX-RS method.
...
When you are done checking out the services, stop the server by using the Maven `liberty:stop-server` goal:
