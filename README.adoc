...
[role='command']
```
mvn liberty:start -pl system,inventory
```
...
Visit the {jaeger-url}[{jaeger-url}^] URL or another location where you configured Jaeger to run and sort the traces
...
the Jaeger server in the form of a user feature for Open Liberty.

This feature is already configured for you in your [hotspot file=1]`pom.xml` and [hotspot file=0]`server.xml` files. It will be
downloaded and installed automatically into each service when you run a Maven build.
...
Because tracing of all JAX-RS methods is enabled by default, you need only to enable [hotspot=mpOpenTracing file=0]`MicroProfile OpenTracing` feature in the `server.xml` file to see some basic traces in Jaeger.
...
```
Point to the
{inv-url}[{inv-url}^] URL, check your Jaeger server, and sort the traces by newest first. You see a new trace record
...
```
Point to the
{inv-url}/localhost[{inv-url}/localhost^] URL, check your Jaeger server, and sort the traces by newest first. You see two new
