...
Visit the {zipkin-url} URL or another location where you configured Zipkin to run. Sort the traces by
newest first and verify that this new trace contains four spans with the following names:
...
the `list()` method in the `InventoryManager` class. Sort by newest first and verify that these spans
...
the remaining `list()` method in the `InventoryManager` class. Sort by newest first and verify that
this span has the following name:
...
custom span that you created around the `addToInventoryList()` call. Sort by newest first and verify
that these spans have the
following names:
