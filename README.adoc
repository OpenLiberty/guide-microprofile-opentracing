...
You will learn how to enable automatic tracing for JAX-RS methods as well as create customer tracers
...
distributed system, allowing developers to tackle otherwise difficult tasks of debugging such requests.
...
Point your browser to http://localhost:9081/inventory/systems. This is the staring point of the `inventory`
...
of one span long will appear in your Zipkin server. Navigate to your Zipkin server to verify this. Next, point
to http://localhost:9081/inventory/systems/localhost.
...
Complete installation instructions for this user feature are available in the
...
Tracing of all JAX-RS applications is enabled by default. Hence, once you enable the MP OpenTracing feature,
tracing of all JAX-RS methods will be automatic without needing to write any additional distributed tracing code.
...
At this point, all you need to do is start the `system` and `inventory` services as well as the Zipkin
server and your traces will appear.
...
to enable tracing with this method:
...
=== Injecting and customizing a Tracer
...
Then, use it to define a new child span in the `list` method around the for loop:
...
Rebuild your application using `mvn package` as before and point to http://localhost:9080/inventory/systems.
A new trace of two spans long will be recorded in your Zipkin server:
