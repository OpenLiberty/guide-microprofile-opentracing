...
Before you proceed, make sure that your Zipkin server is up and running by ensuring that the Zipkin URL
does not return an error with the following curl command:
```
curl {zipkin-url}
```
{: codeblock}
...
When you visit this endpoint, you make two HTTP GET requests, one to the `system` service and one to the `inventory`
...
Both of these requests are configured to be traced, so a new trace will be recorded in Zipkin.
Select **Launch Application** from the menu of the IDE, 
type in **9411** to specify the port number for the Zipkin service, and click the **OK** button. 
Zipkin can be found at the **`https://accountname-9411.theiadocker-4.proxy.cognitiveclass.ai`** URL, 
...
caught by the span. In this case, one of the tests accesses the `/inventory/systems/badhostname`
...
and run the following curl command to access the **inventory** service:
```
...
You see a new trace record that is two spans long with one span for the 
...
You see a new trace record that is just one span long for the remaining [hotspot=list file=0]`list()` 
...
You see two new trace records, one for the `system` service and one for the `inventory` service. The `system` trace 
