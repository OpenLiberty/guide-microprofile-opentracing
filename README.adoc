...
Enable tracing of the [hotspot=list file=0]`list()` non-JAX-RS method by adding [hotspot=Traced file=0]`@Traced` as shown.
...
Now, disable tracing on the `InventoryResource` class.
...
----
[role="edit_command_text"]
Disable tracing of the [hotspot=listContents file=1]`listContents()` JAX-RS method
by setting [hotspot=Traced-false file=1]`@Traced(false)`.
...
[source, Java, linenums, role='code_column hide_tags=copyright,customTracer,Try']
----
...
----
[role="edit_command_text"]
Inject the [hotspot=customTracer file=0]`Tracer`.
Use the `Tracer` to build a [hotspot=span file=0]`Span`.
Then, activate and use the `Span` in a [hotspot=scope file=0]`Scope`. 
