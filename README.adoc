...
https://www.jaegertracing.io/[Jaeger^] is a an open-source distributed tracing system which is compatible with the
...
View the traces for `inventory`. You should see the following trace:
...
You'll need to start the services to see basic traces appear in Jaeger.

When you run Open Liberty in dev mode, the server listens for file changes and automatically recompiles and deploys your
updates whenever you save a new change.
...
Dev mode holds your command line to listen for file changes. Open another command line to continue, or open the project
in your editor.
...
You see a new trace record that is two spans long with one span for the [hotspot=listContents file=1 hide_tags=Traced-false]`listContents()`
...
[source, Java, linenums, role='code_column hide_tags=copyright,addSpan,endTry tags=addToInvList']
----
...
You can use the `@Traced` annotation with a value of `false` to disable the automatic distributed tracing of JAX-RS
methods.
...
When you are done checking out the services, exit development mode by pressing CTRL+C in the shell sessions where you
