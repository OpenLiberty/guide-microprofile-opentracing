...
downloaded and installed automatically into each service when you run a Maven build. You can find the [hotspot=7 file=0]`<feature>`
enabled in your [hotspot file=0]`server.xml` file and the [hotspot=41 file=1]`<artifactId>` in your [hotspot file=1]`pom.xml`.
...
OpenTracing and the Zipkin user feature in the [hotspot file=0]`server.xml` file to see some basic traces in Zipkin.
...
another `@Traced` annotation.
 ...
 in the `inventory/src/main/java/io/openliberty/guides/inventory/InventoryManager.java` file.
...
Enable [hotspot=42 file=0]`tracing` of the [hotspot=43-45 file=0]`list()` non-JAX-RS method.
...
that is two spans long with one span for the [hotspot=41-43 file=1]`listContents()` JAX-RS method in the [hotspot file=1]`InventoryResource`
...
----
[role="edit_command_text"]
Disable tracing of the [hotspot=41-43 file=1]`listContents()` by setting [hotspot=42 file=1]`@Traced(false)`.
