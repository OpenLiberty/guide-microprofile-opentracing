...
The injected [hotspot=customTracer file=0]`Tracer` is used to define a custom span with 
the [hotspot=addSpan file=0]`add()` call.
A [hotspot=childScope file=0]`childScope` is created with this span.
The `childScope` is then activated, and used in a [hotspot=Try file=0]`try` block.
...
Finally, disable tracing on the `InventoryResource` class.
