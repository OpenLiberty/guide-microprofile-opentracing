...
This [hotspot=23 file=1]`Maven plug-in` is responsible for downloading and installing the feature:
...
Enable tracing of the [hotspot=43-45 file=0]`list()` non-JAX-RS method in the `inventory/src/main/java/io/openliberty/guides/inventory/InventoryManager.java` file.
...
class and another span for the [hotspot=43-45 file=0]`list()` method in the `InventoryManager` class. Verify that these spans
