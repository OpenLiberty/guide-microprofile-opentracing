...
Our Zipkin instance can be found at {zipkin-url}.
...
running your services with Docker as well, then ignore the steps in <<Building and running the application>> and
...
containers are running, the `inventory` service can be reached at {inv-url} and the Zipkin server at
{zipkin-url}. To retrieve the JVM system properties of the container running the `system` service, point
to {inv-url}/sys (all containers are linked and therefore they can access each other by their container names).
...
The feature is pulled in and installed automatically using the following Maven plugin in `system/pom.xml` and
`inventory/pom.xml`:
...
long will be recorded in your Zipkin server. The first (parent) Span is for the is the custom `InventoryManager.list`
Span you created earlier and the second is for the new (child) Span that you just created:
...
failure occurs, then you must have introduced a bug into your code. These tests will run automatically
...
No explicit tests exist to verify the correctness of the traces. These can be manually verified by viewing
