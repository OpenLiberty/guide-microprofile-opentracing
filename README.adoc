...
:page-description: You'll explore how to enable and customize tracing of JAX-RS and non-JAX-RS methods by using MicroProfile OpenTracing.
...
You'll explore how to enable and customize tracing of JAX-RS and non-JAX-RS methods by using MicroProfile OpenTracing.
...
MicroProfile OpenTracing enables distributed tracing in MicroProfile applications without adding
any explicit distributed tracing code to the application. Note that the MicroProfile OpenTracing
specification does not address the problem of defining, implementing, or configuring the underlying
distributed tracing system. Rather, the specification makes it easy to instrument services with distributed
tracing given an existing distributed tracing system.
...
to use Zipkin, but keep in mind that you may need additional instructions not listed here if you choose
to use another tracing system.

Before you proceed, make sure your Zipkin server is up and running. By default, Zipkin can be found
...
Make sure your Zipkin server is running and point your browser to the {inv-url}/localhost URL. When you visit
...
You can inspect each span by clicking on it to reveal more detailed information, such as the time
at which the request was received and the time at which a response was sent back.

You might notice a red trace entry in Zipkin, which happens when an error is caught by the span.
In this case, since one of the tests accesses the `/inventory/systems/badhostname` endpoint, which is
invalid, an error is thrown. This behavior is as expected.

When you are done checking out the services, stop both Open Liberty servers:
...
Navigate to the `start` directory and run the `install` and `liberty:start-server` Maven goals:
...
When the servers start, you can find the `system` and `inventory` services at the following locations:
...
To collect traces across your systems, you are required to implement the OpenTracing `Tracer`
...
enabled in your `server.xml` files as:

// This feature is already configured for you and it will be installed automatically into each service
...
Your `pom.xml` files download and install it using the following Maven plugin:
...
This user feature also comes with the `opentracing-1.0` feature, so you don't need to enable it explicitly.
...
The OpenTracing feature (`opentracing-1.0`) found inside Open Liberty enables tracing of all JAX-RS
methods by default, but you can use MicroProfile OpenTracing to control and customize these traces.
Use the `@Traced` annotation to enable and disable tracing of particular methods. You can also inject
a custom `Tracer` object to create and customize spans.
...
Both of these features are already been enabled in the `inventory/src/main/liberty/config/server.xml`
...
If you place the annotation on a class, then its automatically applied to all methods within that class.
...
- The `value=[true|false]` parameter indicates whether or not a particular class or method should
be traced. For example, while all JAX-RS methods are traced by default, you can disable tracing by using
the `@Traced(false)` annotation. This parameter is set to true by default.
- The `operationName=<Span name>` parameter indicates the name of the span that is assigned to the
particular method being traced. If you omit this parameter, the span will be named with the following
form: `<package name>.<class name>.<method name>`. If you place this parameter on a class, all methods
within that class will have the same span name unless they are explicitly overridden with another
`@Traced` annotation.
...
{inv-url} URL and check your Zipkin server. You see a new trace record that is three spans long with
two spans for the `listContents()` JAX-RS method in the `InventoryResource` class and another span for
the `list()` method in the `InventoryManager` class:
...
the remaining `list()` method in the `InventoryManager` class:
...
available for use. You can access the configured `Tracer` by injecting it into a bean using the `@Inject`
...
custom span you created around the `addToInventoryList()` call.
...
This simple example what you can do with the injected `Tracer`. More configuration
...
user feature that is provided. In a real development environment, implement all the necessary OpenTracing
interfaces that you deem necessary, which may include the `SpanBuilder` interface. You can use this
interface for span creation and customization, including setting their timestamps.
...
from the build by using the `mvn verify` command, but first make sure that the servers are stopped.

No explicit tests exist to verify the correctness of the traces. Manually verify these traces by viewing
...
You have just utilized MicroProfile OpenTracing to customize how and which tracers are delivered to Zipkin.
