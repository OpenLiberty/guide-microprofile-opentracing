...
The Maven plugin responsible for downloading and installing the feature is:
// Your `pom.xml` files download and install this feature by using the following Maven plugin:
...
A few tests are included for you to test the basic functionality of the services. not including the
correctness of the traces. If a test failure occurs, then you might have introduced a bug into the code.
These tests will run automatically as a part of the Maven build process when you run the `mvn install`
command. You can also run these tests separately from the build by using the `mvn verify` command, but
first make sure that the servers are stopped.
