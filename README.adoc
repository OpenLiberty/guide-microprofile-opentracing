...
by newest first. Verify that this new trace contains four spans with the following names:
...
InventoryManager.java,lines=44
...
InventoryResource.java,lines=50
...
InventoryManager.java,lines=33
[source, Java, linenums, role="code_column"]
----
...
Next, run the `mvn compile` command from the `start` directory to recompile your services. Point to the
{inv-url}/localhost URL, check your Zipkin server, and sort the traces by newest first. You see a new
trace record that is four spans long with the first two spans for the `getPropertiesForHost()` method
in the `InventoryResource` class and the third span for the `getProperties()` method in the `SystemResource`
class. The last span is the custom span that you created around the `addToInventoryList()` call. Verify
that these spans have the following names:
