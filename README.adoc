...
tracing makes a big difference

MicroProfile OpenTracing aims to enable distributed tracing in MicroProfile applications without having
to explicitly add distributed tracing code to the application.

You will configure the provided `inventory` and `system` services to use MP OpenTracing to enable
automatic tracing of jax-rs methods and create custom tracers for non jax-rs methods.

The complete MicroProfile OpenTracing spec is available https://github.com/eclipse/microprofile-opentracing/blob/master/spec/src/main/asciidoc/microprofile-opentracing.asciidoc[here].
...
Point your browser to http://localhost:9081/inventory/hosts. This is the staring point of the `inventory`
...
to http://localhost:9081/inventory/hosts/localhost.
...
tracing over multiple encapsulated services. If everything were to run a single server, then any logging
...
You must have an OpenTracing Tracer implementation in your application to take advantage
of the MP OpenTracing feature. Luckily, a user feature for Zipkin already exists for you to use.
