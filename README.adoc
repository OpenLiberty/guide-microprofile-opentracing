...
----
[role="edit_command_text"]
Disable tracing of the [hotspot=listContents file=1]`listContents()` JAX-RS method by
setting [hotspot=Traced-false file=1]`@Traced(false)`.
...
Then, use it to build and activate a [hotspot=addSpan file=0]`childScope`. 

The `childScope` is used in a [hotspot=Try file=0]`try` block.
The [hotspot=Try file=0]`try` block that you see here is called a `try-with-resources` statement, 
meaning that the [hotspot=childScope file=0]`childScope` object is closed at the end of the statement. 
