...
When you make this `curl` request, you make two HTTP GET requests, one to the `system` service and one to the `inventory`
service. 
Both of these requests are configured to be traced, so a new trace will be recorded in Zipkin.
Click the `Launch Application` button and enter `9411` as your port to view Zipkin.
Run an empty query and sort the traces by latest start time first. 
endif::[]
...
// cloud-hosted guide instructions:
ifdef::cloud-hosted[]
When the servers start, you can access the `system` and `inventory` services with the following
`curl` commands:
