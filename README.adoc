...
:page-seo-title: Enabling distributed tracing in microservices using Eclipse MicroProfile OpenTracing
:page-seo-description: A tutorial on how to enable distributed tracing in Java microservices to easily trace the flow of a request that spans multiple resources by using MicroProfile OpenTracing and a Zipkin tracing system.
...
downloaded and installed automatically into each service when you run a Maven build. You can find the [hotspot=8 file=0]`opentracingZipkin` feature
enabled in your [hotspot=8 file=0]`server.xml` file.

The [hotspot=191-211 file=1]`download-maven-plugin` Maven plug-in in your `pom.xml` is responsible for downloading and installing the feature.

If you want to install this feature yourself, see
https://www.ibm.com/support/knowledgecenter/SS7K4U_liberty/com.ibm.websphere.wlp.zseries.doc/ae/twlp_dist_tracing.html[Enabling distributed tracing^]
...
OpenTracing and the Zipkin user feature in the [hotspot=7-8 file=0]`server.xml` file to see some basic traces in Zipkin.
...
Enable tracing of the `list()` non-JAX-RS method by updating [hotspot=42 file=0]`@Traced` as shown.
...
that is two spans long with one span for the [hotspot=40-42 file=1]`listContents()` JAX-RS method in the [hotspot file=1]`InventoryResource`
class and another span for the [hotspot=43-45 file=0]`list()` method in the [hotspot file=0]`InventoryManager` class. Verify that these spans
...
----
[role="edit_command_text"]
Disable tracing of the `listContents()` JAX-RS method by setting [hotspot=38 file=1]`@Traced(false)`.
...
{inv-url}[{inv-url}^] URL, check your Zipkin server, and sort the traces by newest first. You see a new trace record
that is just one span long for the remaining [hotspot=43-45 file=0]`list()` method in the `InventoryManager` class. Verify
...
available for use. You can access the configured `Tracer` by injecting it into a bean by using the [hotspot=23 file=0]`@Inject`
...
Then, use it to define a new child scope in the [hotspot=39 file=0]`add()` call.
...
The [hotspot=39-42 file=0]`try` block that you see here is called a `try-with-resources` statement, meaning that the `childScope` object is closed at the end of the statement. It's good practice to define custom spans inside
...
trace contains two spans. The first span is for the [hotspot=26-36 file=2]`getPropertiesForHost()` method in the `InventoryResource` 
class. The second span is the custom span that you created around the [hotspot=38-43 file=0]`add()` call. 
