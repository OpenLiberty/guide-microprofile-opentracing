...
downloaded and installed automatically into each service when you run a Maven build. You can find the [hotspot=7 file=0]`opentracingZipkin` feature
enabled in your [hotspot=8 file=0]`server.xml` file.

The [hotspot=220-240 file=1]`download-maven-plugin` Maven plug-in in your `pom.xml` is responsible for downloading and installing the feature.
