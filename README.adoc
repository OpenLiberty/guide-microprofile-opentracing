...
that is two spans long with one span for the [hotspot=listContents file=1]`listContents()` JAX-RS method in the [hotspot=InventoryResource file=1]`InventoryResource`
class and another span for the [hotspot=list file=0]`list()` method in the [hotspot=InventoryManager file=0]`InventoryManager` class. Verify that these spans
