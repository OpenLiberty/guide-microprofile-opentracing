...
Enable tracing of the `list()` non-JAX-RS method by updating [hotspot=Traced file=0]`@Traced` as shown.

InventoryManager.java
[source, Java, linenums, role='code_column hide_tags=copyright,custom-tracer,addToInvList']
----
...
----
[role="edit_command_text"]
Disable tracing of the `listContents()` JAX-RS method by setting [hotspot=Traced-false file=1]`@Traced(false)`.
...
available for use. You can access the configured `Tracer` by injecting it into a bean by using the [hotspot=custom-tracer file=0]`@Inject`
...
Then, use it to define a new child scope in the [hotspot=try file=0]`add()` call.
...
The [hotspot=tag::custom-tracer file=0]`try` block that you see here is called a `try-with-resources` statement, meaning that the `childScope` object is closed at the end of the statement. It's good practice to define custom spans inside
...
contains one span for the [hotspot=getProperties file=1]`getProperties()` method in the `SystemResource` class. The `inventory` 
