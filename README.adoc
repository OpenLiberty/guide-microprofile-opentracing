...
----
include::finish/inventory/src/main/liberty/config/server.xml[tags=**]
----
...
This feature is already configured for you in your [hotspot=1-267 file=1]`pom.xml` and [hotspot=1-26 file=0]`server.xml` files. It will be
downloaded and installed automatically into each service when you run a Maven build. You can find it
enabled in your [hotspot=10 file=0]`server.xml` file.

This [hotspot=229-247 file=1]`Maven plug-in` is responsible for downloading and installing the feature:
...
OpenTracing and the Zipkin user feature in the [hotspot=1-25 file=0]`server.xml` file to see some basic traces in Zipkin.

Both of these features are already enabled in the [hotspot=9-10 file=0]`inventory/src/main/liberty/config/server.xml`
and [hotspot=9-10 file=1]`system/src/main/liberty/config/server.xml` files:
...
[source, xml,  linenums, role="code_column"]
----
include::finish/system/src/main/liberty/config/server.xml[tags=**]
----
...
Use the [hotspot=42 file=0]`@Traced` annotation to define explicit span creation for specific classes and methods.
...
The [hotspot=42 file=0]`@Traced` annotation can be configured with the following two parameters:
...
Enable tracing of the [hotspot=48-50 file=0]`list()` non-JAX-RS method in the `inventory/src/main/java/io/openliberty/guides/inventory/InventoryManager.java` file.

[role="code_command hotspot=42 file=0", subs="quotes"]
----
Add the `Traced` annotation to the `InventoryManager` class.
...
that is two spans long with one span for the [hotspot=43-45 file=1]`listContents()` JAX-RS method in the `InventoryResource`
class and another span for the [hotspot=48-50 file=0]`list()` method in the `InventoryManager` class. Verify that these spans
...
{inv-url} URL, check your Zipkin server, and sort the traces by newest first. You see a new trace record
that is just one span long for the remaining [hotspot=43-35 file=0]`list()` method in the `InventoryManager` class. Verify
...
available for use. You can access the configured `Tracer` by injecting it into a bean by using the [hotspot=25 file=0]`@Inject`
annotation from the Contexts and Dependency Injections API.

[role="code_command hotspot=25-26 file=0", subs="quotes"]
----
Add the `Tracer` object into the `InventoryManager` file.
`inventory/src/main/java/io/openliberty/guides/inventory/InventoryManager.java`
----

[role="code_command hotspot=40-44 file=0", subs="quotes"]
----
Define a new child span in the `get()` method around the `add()` call.
`inventory/src/main/java/io/openliberty/guides/inventory/InventoryManager.java`
----
...
contains one span for the [hotspot=19-21 file=1]`getProperties()` method in the `SystemResource` class. The `inventory` 
trace contains two spans. The first span is for the [hotspot=27-38 file=2]`getPropertiesForHost()` method in the `InventoryResource` 
class. The second span is the custom span that you created around the [hotspot=40-44 file=0]`add()` call. 
