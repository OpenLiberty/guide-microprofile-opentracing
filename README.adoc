...
The following Maven plugin is responsible for downloading and installing the feature:
...
custom span that you created around the `addToInventoryList()` call.
...
A few tests are included for you to test the basic functionality of the services, but they do not
test the correctness of the traces. If a test failure occurs, then you might have introduced a bug
into the code. These tests will run automatically as a part of the Maven build process when you run the `mvn install`
