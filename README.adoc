...
Before you continue, make sure your Zipkin server is up and running. Select **Launch Application** from the menu of the IDE and type **9411** to specify the port number for the Zipkin service. Click the **OK** button. Zipkin can also be found at the **`https://accountname-9411.theiadocker-4.proxy.cognitiveclass.ai`** URL, where **accountname** is your account name.
endif::[]
...
When you make this curl request, you make two HTTP GET requests, one to the **system** service and one to the **inventory** service. Because tracing is configured for both these requests, a new trace is recorded in Zipkin. Visit the **`https://accountname-9411.theiadocker-4.proxy.cognitiveclass.ai`** URL. Run an empty query and sort the traces by latest start time first. 
endif::[]
