...
OpenTracing and the Zipkin user feature in the [hotspot=mpOpenTracing file=0]`server.xml` file to see some basic traces in Zipkin.
...
Enable tracing of the [hotspot=`list()` non-JAX-RS method by updating [hotspot=Traced file=0]`@Traced` as shown.

InventoryManager.java
[source, Java, linenums, role='code_column tags=addToInvList hide_tags=copyright,customTracer']
----
...
user features that is provided. In a real-world scenario, implement all the OpenTracing interfaces that
